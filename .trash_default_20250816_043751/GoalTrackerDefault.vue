    ...
    >
      <div class="max-w-xl mx-auto px-4 pt-2 pb-3">
        <div class="flex items-center justify-between">
          <h1 class="text-2xl font-bold text-cyan-300">🎯 Goal Tracker</h1>
        ...
        <div class="mt-2 grid grid-cols-3 gap-2 text-[11px]">
          <div class="stat"><p class="text-white/70">Active</p><p class="text-cyan-300 font-semibold">{{ stats.active }}</p></div>
          <div class="stat"><p class="text-white/70">Completed</p><p class="text-emerald-300 font-semibold">{{ stats.completed }}</p></div>
          <div class="stat"><p class="text-white/70">Avg. Progress</p><p class="text-amber-300 font-semibold">{{ stats.avg }}%</p></div>
        </div>
      </div>
    </header>
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div
              ...
              role="progressbar" aria-label="Progress" :aria-valuenow="g.progress" aria-valuemin="0" aria-valuemax="100"
            >
            ...
            <div class="hidden sm:flex items-center gap-1">
              ...
              <button class="icon-btn" title="Edit" @click="openSheet(g)">✏️</button>
            </div>
          ...
          <div class="mt-3 flex items-center gap-2 text-[12px]">
            <span class="text-white/60">{{ g.completed ? 'Completed' : (g.due ? ('Due ' + relative(g.due)) : 'No due date') }}</span>
...
const list = ref([])              // paginated render list
...
function save(){
  ...
  setTimeout(()=> {
    if (editing.value){
      ...
      all.value.unshift({ ...form, id, createdAt: Date.now(), updatedAt: Date.now() })
    }
    saving.value = false
    sheet.open = false
    vibrate(10)
  ...
  io = new IntersectionObserver(entries=>{
    entries.forEach(e=>{
      if (e.isIntersecting && !moreLoading.value && !loading.value){
        ...
        setTimeout(()=> { paginate(); moreLoading.value = false }, 250)
      }
    })
