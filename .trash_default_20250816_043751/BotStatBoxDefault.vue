<!-- StatChipMobilePro.vue -->
<template>
  <!-- Button semantics when clickable for better mobile a11y -->
  <component
    :is="clickable ? 'button' : 'div'"
    class="w-full max-w-sm mx-auto rounded-2xl p-4 sm:p-5 shadow-md transition active:scale-[0.99]
...
/**
 * Tailwind-safe palette (no dynamic class strings that get purged).
...
const props = defineProps({
  ...
  icon: { type: String, default: '' },          // emoji or short icon
...
const ariaLabel = computed(() => `${props.label}: ${typeof props.value === 'number' ? formattedValue.value : props.value}`)
  ...
  switch (props.format) {
    ...
    default:
      return Math.abs(n) >= 1000
        ? new Intl.NumberFormat(props.locale, { notation: 'compact', maximumFractionDigits: 1 }).format(n)
        : new Intl.NumberFormat(props.locale).format(n)
  }
})

const trendClasses = computed(() => {
  if (props.trend === 'up')   return { badge: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200' }
  if (props.trend === 'down') return { badge: 'bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-200' }
...
<style scoped>
/* purely Tailwind, no extra CSS required */
</style>
