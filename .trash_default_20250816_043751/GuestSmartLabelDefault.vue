<template>
  <transition name="fade">
    <div v-if="visible" class="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 bg-black/80 text-white px-6 py-3 rounded-xl shadow-lg text-sm text-center max-w-xs">
      <span>{{ label }}</span>
    </div>
  </transition>
</template>
