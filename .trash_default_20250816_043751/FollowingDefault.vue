<template>
  ...
  >
    ...
    <header
      class="sticky top-0 z-30 bg-gradient-to-b from-[#0f172a]/90 to-[#0f172a]/60 backdrop-blur-lg border-b border-cyan-800/40"
    ...
    >
      <div class="max-w-xl mx-auto px-4 pt-2 pb-3">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs text-white/70" aria-live="polite">{{ prettyCount }} account{{ totalCount!==1 ? 's' : '' }}</span>
        </div>
        ...
        <div class="flex gap-2">
          ...
          <div class="relative">
            <button class="chip" @click="toggleSortMenu" :aria-expanded="String(sortOpen)">
              {{ sortLabel }}
            </button>
        ...
        <!-- Offline banner -->
        <div v-if="!online" class="mt-2 text-[11px] text-amber-200 bg-amber-500/10 border border-amber-400/30 rounded-lg px-2 py-1">
          You’re offline — changes will sync when back online.
        </div>

        <!-- Selection mode toolbar -->
        <div v-if="selectMode" class="mt-2 flex items-center gap-2 text-[13px]">
          <span class="px-2 py-1 rounded bg-white/10 border border-white/10">Selected: {{ selectedCount }}</span>
          <button class="btn-danger px-3 py-1" @click="bulkUnfollow" :disabled="selectedCount===0">Unfollow</button>
          <button class="btn-cyan px-3 py-1" @click="bulkMessage" :disabled="selectedCount===0">Message</button>
          <button class="btn-dark px-3 py-1 ml-auto" @click="exitSelect">Done</button>
        </div>
      </div>
    </header>
    ...
    <div
      class="sticky top-[calc(-1*env(safe-area-inset-top))] z-20 flex items-center justify-center h-10 text-xs text-white/70"
      :style="{ marginTop: pull.offset + 'px' }"
      aria-hidden="true"
    >
      <div v-if="pull.offset>0" class="px-2 py-1 rounded-full bg-white/10 border border-white/10">
        <span v-if="!pull.ready">Pull to refresh…</span>
        <span v-else>Release to refresh</span>
      </div>
    </div>
        ...
        >
          ...
          <div class="absolute inset-0 rounded-xl overflow-hidden">
            <div class="w-full h-full grid grid-cols-2">
              <div></div>
              <div class="flex items-center justify-end pr-3 bg-rose-600/20">
                <span class="text-rose-300 text-sm">Unfollow →</span>
              </div>
            </div>
          </div>

          <!-- Card (swipeable container) -->
          <div
            class="group bg-[#1e293b]/80 p-3 rounded-xl border border-cyan-700/50 hover:border-cyan-500/60 shadow-sm hover:shadow-lg transition will-change-transform"
            :style="swipeStyle(u)"
            @pointerdown="onSwipeStart($event, u)"
            @pointermove="onSwipeMove($event)"
            @pointerup="onSwipeEnd"
            @pointercancel="onSwipeEnd"
            @pointerleave="onSwipeLeave"
            @contextmenu.prevent
            @touchstart.passive="onLongPressStart(u)"
            @touchend.passive="onLongPressEnd"
            @mousedown.passive="onMousePressStart(u)"
            @mouseup.passive="onMousePressEnd"
          >
            <div class="flex items-center gap-3">
              <div class="relative">
                ...
                <button
                  v-if="selectMode"
                  class="absolute -top-1 -left-1 w-5 h-5 rounded-full grid place-items-center bg-white/10 border border-white/20"
                  :aria-pressed="isSelected(u)"
                  @click.stop="toggleSelect(u)"
                >
                  <span v-if="isSelected(u)">✓</span>
                </button>
              </div>
              ...
              <button class="icon-btn sm:hidden" aria-label="More" @click.stop="openSheet(u)">⋯</button>
            </div>
          </div>
        </article>
      </div>
...
/* Persist user prefs */
const PREFS_KEY = 'following:prefs'
onMounted(() => {
  try{
    const p = JSON.parse(localStorage.getItem(PREFS_KEY) || '{}')
    if (p.role) activeRole.value = p.role
    if (p.sort) sortBy.value    = p.sort
  }catch{}
})
watch([activeRole, sortBy], ([r,s])=>{
  try{ localStorage.setItem(PREFS_KEY, JSON.stringify({role:r, sort:s})) }catch{}
})
...
function unfollow(u){
  ...
  confirm.open = false
  selected.value.delete(u.username)
  emit('unfollow', u)
}
...
function isSelected(u){ return selected.value.has(u.username) }
function toggleSelect(u){
  if (selected.value.has(u.username)) selected.value.delete(u.username)
  else selected.value.add(u.username)
}
function enterSelect(u){ selectMode.value = true; selected.value.add(u.username) }
function exitSelect(){ selectMode.value = false; selected.value.clear() }
function bulkUnfollow(){
  const toRemove = [...selected.value]
  toRemove.forEach(username=>{
    const u = all.value.find(x=> x.username===username)
    if (u) unfollow(u)
  })
  exitSelect()
}
function bulkMessage(){
  const users = [...selected.value].map(u => all.value.find(x=> x.username===u)).filter(Boolean)
  if (users.length) emit('message', users)
  exitSelect()
}

/* Long-press handlers (mobile + mouse fallback) */
let lpTimer = null
function onLongPressStart(u){
  if (selectMode.value) return
  lpTimer = setTimeout(()=> { enterSelect(u); vibrate(12) }, 420)
}
function onLongPressEnd(){ clearTimeout(lpTimer); lpTimer = null }
function onMousePressStart(u){
  if (window.matchMedia('(pointer: coarse)').matches) return
  onLongPressStart(u)
}
function onMousePressEnd(){ onLongPressEnd() }
...
function vibrate(ms){ if (navigator.vibrate && !prefersReducedMotion.value) try{ navigator.vibrate(ms) }catch{} }
function onAvatarError(e){ e.target.src = '/user-avatar.png' }

/* Prefers-reduced-motion */
const prefersReducedMotion = ref(false)
onMounted(()=>{ prefersReducedMotion.value = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false })
...
/* Refresh (public + pull-to-refresh) */
...
const pull = reactive({ startY:0, offset:0, ready:false, tracking:false })
...
function onTouchMove(e){
  ...
  if (dy > 0){
    pull.offset = Math.min(100, dy * 0.6)
    pull.ready = pull.offset > 70
  }
}
async function onTouchEnd(){
  if (!pull.tracking) return
  const doRefresh = pull.ready
  pull.tracking = false
  pull.ready = false
  pull.offset = 0
...
/* Swipe-to-unfollow (per active card) */
const swipe = reactive({ key:null, startX:0, dx:0, active:false })
function onSwipeStart(e, u){
  if (selectMode.value) return // disable while selecting
  swipe.key = u.username
  swipe.startX = e.clientX ?? e.touches?.[0]?.clientX ?? 0
  swipe.dx = 0
  swipe.active = true
...
function onSwipeMove(e){
  ...
  swipe.dx = Math.min(0, x - swipe.startX) // only left
}
function onSwipeEnd(){
  if (!swipe.active) return
  const threshold = -90
  ...
  swipe.dx = 0
  if (dx <= threshold){
    const u = all.value.find(x=> x.username===key)
    if (u) askUnfollow(u)
  }
}
function onSwipeLeave(){ if (swipe.active) onSwipeEnd() }
function swipeStyle(u){
  if (swipe.active && swipe.key === u.username){
    const clamped = Math.max(-120, Math.min(0, swipe.dx))
    return { transform: `translateX(${clamped}px)`, transition: 'none' }
  }
  return { transform: 'translateX(0)', transition: 'transform .2s ease' }
}
...
const handleDocClick = (e)=> {
  ...
  if (!e.target.closest || !e.target.closest('.menu') && !e.target.closest('.chip')) sortOpen.value = false
}
onMounted(async ()=>{
  await seedData()
  loading.value = false
  ...
  rootEl.value?.addEventListener('touchmove',  onTouchMove,  { passive:true })
