  ...
  <div class="page bg-dark d-flex align-items-center justify-content-center px-3">
    <div class="wrap" style="width:100%;max-width:520px">
      <div class="text-center mb-3 mt-2">
        <img src="/icons/logo.png" alt="SmartBiz Logo" width="56" height="56"
             class="rounded-circle border border-warning shadow-sm"
             style="background:#fff;object-fit:contain" loading="eager" decoding="async" />
        ...
        <form @submit.prevent="resetPassword" autocomplete="off" novalidate :aria-busy="loading">
          ...
          <div class="mb-2">
            ...
            <div class="input-group group-dark rounded-3 overflow-hidden">
              ...
              <input
                ...
                required
              />
              <button class="btn btn-outline-warning btn-toggle" type="button"
                      :aria-pressed="showPwd" :title="showPwd ? 'Hide password' : 'Show password'"
                      @click="showPwd = !showPwd" :disabled="loading">
            ...
            <div class="mt-2">
              ...
              <small v-if="password" class="d-block mt-1"
                     :class="isStrongPassword(password) ? 'text-success' : 'text-danger'">
                {{ strength.label }}
              </small>
            </div>
          ...
          <div class="mb-2">
            ...
            <div class="input-group group-dark rounded-3 overflow-hidden">
              ...
              />
              <button class="btn btn-outline-warning btn-toggle" type="button"
                      :aria-pressed="showConfirm" :title="showConfirm ? 'Hide password' : 'Show password'"
                      @click="showConfirm = !showConfirm" :disabled="loading">
          ...
          <div class="text-center mt-3 small text-secondary">
            ...
            <router-link to="/login" class="text-warning fw-bold text-decoration-underline ms-1">
              Back to Login
            </router-link>
          </div>
        </form>
...
/* ───── State ───── */
const password        = ref<string>(''), confirmPassword = ref<string>('')
const showPwd = ref(false), showConfirm = ref(false), capsOn = ref(false)
const loading = ref(false), error = ref(''), success = ref(''), hint = ref('')
...
const maskedIdentifier = computed(() => {
  ...
  if (id.includes('@')) { const [u,d] = id.split('@'); return `${u.slice(0,2)}***@${d}` }
...
function getResetToken(): string {
  const urlT = new URLSearchParams(location.search).get('token') || ''
  const mem  = sessionStorage.getItem('reset_token') || localStorage.getItem('reset_token') || ''
  return urlT || mem
}
...
onMounted(() => { hint.value = `API base: ${BACKEND_BASE}` })

/* ───── Validation + strength ───── */
const reUpper=/[A-Z]/, reLower=/[a-z]/, reDigit=/\d/, reSymbol=/[^\w\s]/
...
const strength = computed(()=>{
  ...
  if (p.length>=8) s++; if (reUpper.test(p)) s++; if (reLower.test(p)) s++; if (reDigit.test(p)) s++; if (reSymbol.test(p)) s++
  const map=[{label:'Too weak',percent:15,barClass:'bg-danger'},
...
function onPwdKey(e:KeyboardEvent){ try{ capsOn.value=!!(e.getModifierState&&e.getModifierState('CapsLock')) }catch{ capsOn.value=false }}

/* ───── Submit ───── */
function parseError(e:any){
  if (!e?.status && /network/i.test(e?.message||'')) return 'Network/CORS error. Ensure your backend allows this origin.'
  return e?.data?.detail || e?.data?.message || e?.message || 'Failed to reset password.'
}
const ENDPOINTS=['/auth/reset-password','/api/auth/reset-password','/api/auth/password/reset','/api/password/reset']

async function resetPassword(){
  if (loading.value || !resetToken.value) return
  if (!online.value){ error.value='You appear to be offline.'; return }
  if (!canSubmit.value){ error.value='Please meet all password requirements and ensure the passwords match.'; return }
  ...
  try{
    ...
    for (const url of ENDPOINTS){
      for (const body of payloads){
        ...
        catch(e:any){ last=e; const st=e?.status; if (st && ![404,405,415].includes(st)) break }
      }
      if (ok) break
    }
    if (!ok) throw last || new Error('No reset route responded.')

    sessionStorage.removeItem('reset_token'); localStorage.removeItem('reset_token'); sessionStorage.removeItem('reset_identifier')
...
.card{ box-shadow:0 8px 28px rgba(0,0,0,.4),0 0 0 2px #ffd70033; border:2px solid #ffd700 !important; transition:box-shadow .3s ease, transform .2s ease; }
.card:hover{ transform:translateY(-2px); box-shadow:0 10px 32px rgba(0,0,0,.5),0 0 0 2px #ffd70066; }
.group-dark .input-group-text, .group-dark .form-control{ background:#132441 !important; color:#f8f9fa !important; border:none !important; }
.group-dark .form-control::placeholder{ color:#b9c3d3 !important; }
.group-dark .form-control:focus{ outline:none !important; box-shadow:0 0 0 2px #ffd70099 !important; }
.btn-toggle{ border-left:1px solid rgba(255,255,255,.08) !important; }
.btn-warning{ background:#ffd700 !important; color:#0b1220 !important; border:none !important; font-weight:700; border-radius:.65rem; transition:all .2s ease; }
.btn-warning:hover{ background:#ffdd33 !important; box-shadow:0 0 10px rgba(255,214,0,.4); }
.btn-warning:active,.btn-warning:focus{ background:#ffec80 !important; outline:none !important; }
