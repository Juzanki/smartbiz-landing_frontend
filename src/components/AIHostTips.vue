...
<script setup>
/**
 * Mobile-first AI Host Tips bottom sheet
 * - Topic + category filters
 * - Swipeable carousel, dots, pager
 * - Save / Copy / Share / Dismiss
 * - LocalStorage for saved & dismissed
 * - Haptics (navigator.vibrate) + a11y live region
 */
...
/* Generative (client-side) */
async function generate(){
  generating.value = true
  vibrate(8)
  await new Promise(r => setTimeout(r, 450)) // simulate AI latency
  ...
  if (out.length < 5) out = pool.slice(0, 7)
...
/* Tip pool (could be replaced by real AI) */
function basePool(topicText){
  ...
  const items = [
    ...
    tip('💸', 'Micro-CTA for Gifts', niched('“When the counter hits 5 gifts, I’ll reveal the shortcut to {topic}.”'), ['monetization','engagement'], [
      'Set a tiny target (e.g., 3 gifts) to start momentum',
    ...
    tip('🎛️', 'Segment Switch', niched('Every 6–8 minutes switch format: demo → Q&A → recap on {topic}.'), ['content','engagement'], [
      ...
      'Announce next segment to reduce drop-off'
    ]),
    ...
    tip('⏱️', '90-Second Loop', niched('Repeat the core promise every ~90s for scrollers joining {topic}.'), ['content','growth'], [
      ...
      'Place the best nugget every 2–3 minutes'
    ]),
    tip('🧭', 'Outcome > Features', niched('Frame everything as outcomes: “This gets you X in Y time” for {topic}.'), ['content'], [
      'Use numbers: “3 steps… 10 mins…”',
      'Contrast before vs after quickly'
    ]),
  ]

  // add unique fingerprint for dedupe
  return items.map(i => ({ ...i, fingerprint: hash(i.title + i.body) }))
}

function tip(icon, title, body, tags, prompts){
  return { icon, title, body, tags, prompts }
}
function hash(s){
