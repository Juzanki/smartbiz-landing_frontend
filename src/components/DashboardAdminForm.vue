...
<template>
  <form
    class="admin-form relative mx-auto bg-white/90 dark:bg-white/5 rounded-2xl shadow-xl border border-white/20 max-w-md w-[min(94vw,40rem)] overflow-hidden backdrop-blur"
    :style="{ '--accent': accent }"
    autocomplete="off"
    novalidate
  ...
  >
    <!-- Header -->
    <header class="px-4 sm:px-6 pt-5 pb-4 bg-gradient-to-r from-[color-mix(in_oklab,var(--accent)_70%,white)]/10 to-transparent">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="text-[.85rem] text-slate-500 dark:text-slate-300 line-clamp-1">
            Configure your business identity and contact details
          </p>
        </div>
      </div>
      ...
      <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
        <span v-if="!online" class="chip chip-warn">
          <i class="i-tabler-wifi-off"></i> Offline (changes saved locally)
        </span>
        <span v-else class="chip chip-ok">
          <i class="i-tabler-wifi"></i> Online
        </span>
    ...
    <section class="px-4 sm:px-6 py-5 grid gap-4">
      ...
      <div>
        ...
        <div class="field">
          ...
          <input
            ...
            type="text"
            inputmode="text"
            placeholder="e.g., SmartBiz Assistance"
      ...
      <div>
        ...
        <div class="field">
          ...
          />
          <button type="button" class="suffix-btn" @click="toggleWhatsApp = !toggleWhatsApp" :aria-pressed="toggleWhatsApp">
        ...
        <p class="hint">Use international format without spaces (auto-formats). Example: +2557XXXXXXXX</p>
      </div>

      <!-- Description -->
      <div>
        <label for="businessDesc" class="label">Short Description <span class="muted">(optional)</span></label>
      ...
      <transition name="fade">
        <div
          ...
          class="rounded-xl border border-red-200/60 bg-red-50/80 text-red-700 text-sm px-3 py-2"
        >
      ...
      <transition name="fade">
        <div
          v-if="successMsg"
          class="rounded-xl border border-green-200 bg-green-50 text-green-700 text-sm px-3 py-2"
        >
          {{ successMsg }}
        </div>
      </transition>
    ...
    <footer
      class="sticky-actions px-4 sm:px-6 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 bg-white/85 dark:bg-slate-900/70 backdrop-blur border-t border-white/20 flex items-center gap-3"
    >
      <button
        type="button"
      ...
      <button
        ...
        :disabled="loading || !dirty || hasErrors || !online"
      >
...
const props = defineProps({
  ...
  accent: { type: String, default: '#f59e0b' }, // SmartBiz yellow by default
  autosaveKey: { type: String, default: 'dashboard_admin_form_draft' }
})
...
const toggleWhatsApp = ref(true) // suffix icon toggle
...
function touch(key) { touched[key] = true }
...
const hasErrors = computed(() =>
  !!errors.businessName || !!errors.businessEmail || !!errors.businessPhone || !!errors.businessDesc
)
...
function validate() {
  ...
  errors.businessEmail = !form.businessEmail
    ? 'Email is required.'
    : !emailOk
      ? 'Invalid email format.'
      : ''

  // Phone (+country + digits; Tanzania sample +2557…; 8–15 digits after +)
  const phoneDigits = (form.businessPhone || '').replace(/\s+/g, '')
  const phoneOk = /^\+\d{8,15}$/.test(phoneDigits)
  errors.businessPhone = !form.businessPhone
    ? 'Phone is required.'
    : !phoneOk
      ? 'Use international format, e.g., +2557XXXXXXXX.'
      : ''

  // Desc
  errors.businessDesc =
    form.businessDesc && form.businessDesc.length > descMax
      ? `Description is too long (max ${descMax}).`
      : ''
}
...
/* Phone formatting (display with spaces, store normalized) */
...
function formatPhone(e) {
  ...
  raw = raw.replace(/(?!^)\+/g, '')
  // beautify spacing for +2557 XXX XXX XXX style
  const cleaned = raw.startsWith('+2557')
    ? raw.replace(/^(\+2557)(\d{3})(\d{3})(\d{0,3}).*/, (_, a, b, c, d) =>
        d ? `${a} ${b} ${c} ${d}` : c ? `${a} ${b} ${c}` : b ? `${a} ${b}` : a
      )
    : raw.replace(/^(\+\d{1,3})(\d{3})(\d{3})(\d{0,3}).*/, (_, a, b, c, d) =>
        d ? `${a} ${b} ${c} ${d}` : c ? `${a} ${b} ${c}` : b ? `${a} ${b}` : a
      )
  phoneDisplay.value = cleaned
  form.businessPhone = cleaned.replace(/\s+/g, '')
}
...
function autosave() {
  ...
  saveTimer = setTimeout(() => {
    try {
      localStorage.setItem(props.autosaveKey, JSON.stringify(form))
    } catch {}
  }, 400)
}
function restoreDraft() {
  try {
    ...
    const isDifferent = JSON.stringify(draft) !== JSON.stringify(form)
    if (isDifferent) {
...
function clearDraft() {
  try { localStorage.removeItem(props.autosaveKey) } catch {}
}
...
async function onSubmit() {
  ...
  validate()
  if (hasErrors.value) {
    showToast('Please fix errors', '⚠️')
    return
  }
  loading.value = true
  successMsg.value = ''
  try {
    // Emit to parent (replace with API call if needed)
...
/* ---------- tiny design system ---------- */
.label {
  @apply block text-[.85rem] font-medium text-slate-800 dark:text-slate-200 mb-1;
}
.field {
  @apply relative flex items-center rounded-xl border bg-white/70 dark:bg-slate-900/60 backdrop-blur px-3 py-2
         border-slate-200/80 dark:border-white/10 focus-within:border-[var(--accent)]/60 focus-within:ring-2 focus-within:ring-[var(--accent)]/30;
}
.field-icon {
  @apply text-slate-500 dark:text-slate-300 mr-2;
}
.input {
  @apply w-full bg-transparent outline-none text-slate-900 dark:text-white placeholder:text-slate-400/80;
}
.textarea-wrap { @apply items-start; }
.textarea { @apply resize-y min-h-[84px] pr-12; }
.counter {
  @apply absolute right-3 bottom-2 text-[.72rem] text-slate-500/80;
}
.prefix {
  @apply absolute left-3 text-slate-500 select-none;
}
.suffix-btn {
  @apply absolute right-2 grid place-items-center h-8 w-8 rounded-lg text-slate-600 dark:text-slate-200 hover:bg-black/5 dark:hover:bg-white/5;
}

.error {
  @apply mt-1 text-[.8rem] text-red-600;
}
.hint {
  @apply mt-1 text-[.78rem] text-slate-500;
}
.muted { @apply text-slate-400; }

.chip {
  @apply inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[.72rem] font-medium;
}
...
/* Buttons */
.btn {
  @apply inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 font-bold transition active:translate-y-[1px] select-none;
}
.btn.primary {
  color: white;
  background: color-mix(in oklab, var(--accent) 94%, black);
...
.btn.ghost {
  @apply text-slate-700 dark:text-slate-100 border border-slate-200/70 dark:border-white/10 bg-white/60 dark:bg-white/5;
}
...
/* Toasts */
.toasts { position: fixed; left: 1rem; bottom: calc(4.5rem + env(safe-area-inset-bottom)); display: grid; gap: 6px; z-index: 50; }
.toast {
  @apply bg-slate-900/90 text-white text-sm px-3 py-2 rounded-xl border border-white/10 shadow-xl;
}
