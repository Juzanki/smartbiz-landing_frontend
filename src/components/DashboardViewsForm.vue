...
<template>
  ...
  >
      ...
      <div class="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
        <span
          :class="[
            ...
            online ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'
          ]"
        >
    ...
    <div>
      ...
      <p class="mt-1 text-[12px] text-slate-500">{{ $t('intl_format_note') || 'International format stored (e.g., +2557XXXXXXXX).' }}</p>
    </div>
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <input
        id="notif" type="checkbox" v-model="form.notifications"
...
function validate(){
  ...
  // phone (optional, but must be intl if present)
...
const passScore = ref(0) // 0..100
const passLabel = computed(()=>{
  if (passScore.value >= 80) return 'Strong password'
  if (passScore.value >= 55) return 'Medium password'
  if (passScore.value > 0)   return 'Weak password'
  return ''
})
const passBarClass = computed(()=>{
  return passScore.value >= 80 ? 'bg-emerald-500' : passScore.value >= 55 ? 'bg-amber-500' : 'bg-rose-500'
})
const passTextClass = computed(()=>{
  return passScore.value >= 80 ? 'text-emerald-600' : passScore.value >= 55 ? 'text-amber-600' : 'text-rose-600'
})
...
async function onSubmit(){
  ...
  loading.value = true
  try{
    // Simulated backend (replace with real API)
    ...
    clearDraft()
    form.password = ''  // clear after save
