<template>
  ...
  >
    ...
    <header class="flex items-center gap-3 mb-3">
      ...
      <div class="min-w-0">
        ...
        <div class="mt-1 flex flex-wrap items-center gap-2 text-[11px]">
          <span :class="['px-2 py-0.5 rounded-full border', online ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200']">
    ...
    <p v-if="form.notifications && notifPerm==='default'" class="text-[12px] text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded">
      Tip: enable browser notifications for real-time alerts. <button class="underline font-medium" @click="requestPush">Enable</button>
    </p>

    <!-- Success -->
    <transition name="fade">
      <div v-if="successMsg"
           class="text-green-700 bg-green-50 border border-green-200 px-3 py-2 rounded text-center font-medium shadow-sm">
        {{ successMsg }}
      </div>
    </transition>
    ...
    <footer class="sticky bottom-0 bg-gradient-to-t from-white to-transparent pt-3"
      ...
      <div class="flex items-center gap-2">
        <button type="button" class="flex-1 border rounded-lg px-4 py-2 font-semibold"
...
const props = defineProps({
  ...
  accent: { type: String, default: '#ffd600' },
  autosaveKey: { type: String, default: 'user_profile_form_draft' }
})
...
function validate(){
  ...
  const okPhone = ph === '' || /^\+\d{8,15}$/.test(ph)  // optional; if present must be intl
  errors.phone = okPhone ? '' : 'Use +country then digits, e.g., +2557XXXXXXXX.'
}

/* Phone formatting (pretty display; store normalized) */
...
async function onPickAvatar(e){
  ...
  if (file.size > 2 * 1024 * 1024) { // 2MB
    successMsg.value = 'Image too large (max 2MB)'
    setTimeout(()=> successMsg.value = '', 1800)
    return
  }
...
async function copyEmail(){
  try { await navigator.clipboard.writeText(form.email || ''); ping('Email copied') } catch {}
}

/* Notifications permission hint */
const notifPerm = ref(typeof Notification !== 'undefined' ? Notification.permission : 'unsupported')
async function requestPush(){
  if (typeof Notification === 'undefined') return
  try { const p = await Notification.requestPermission(); notifPerm.value = p }
  catch {}
}
...
/* Success ping */
const successMsg = ref('')        // inline banner
function ping(msg){ successMsg.value = msg; setTimeout(()=> successMsg.value = '', 1800) }
...
watch(() => props.userData, (val) => {
  Object.assign(form, {
    ...
    avatarDataUrl: val?.avatarDataUrl || ''
  })
  phoneDisplay.value = form.phone || ''
  Object.assign(base, JSON.parse(JSON.stringify(form)))
}, { deep: true })
