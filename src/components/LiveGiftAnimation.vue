  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-2 min-w-0">
        ...
        <button
          class="ml-1 px-2 py-0.5 bg-yellow-500/20 text-yellow-200 rounded-full text-[10px] shrink-0"
          @click.stop="emit('open-ranking')"
        >🔥 Daily Ranking</button>

        <button
          class="ml-1 px-2 py-0.5 bg-pink-600 text-white rounded-full text-[10px] hover:bg-pink-700 shrink-0"
          @click.stop="showAddGoal = true"
        >➕ Add Goal</button>
      </div>
    ...
    <div class="absolute top-20 left-1/2 -translate-x-1/2 z-40 text-center">
      ...
      <div
        v-if="currentGoal"
        class="mt-2 bg-gradient-to-r from-yellow-400 via-orange-500 to-pink-500 text-white text-[11px] px-3 py-1 rounded-full shadow backdrop-blur border border-white/20 animate-pulse"
      >
        🎯 Goal: {{ currentGoal }}
      </div>
    </div>
    ...
    <transition name="fade">
      <div
        v-if="showAiHostAvatar"
        class="absolute bottom-[160px] right-3 z-50 bg-white/10 border border-white/20 p-2.5 rounded-xl backdrop-blur text-center"
      >
    ...
    <component :is="LiveGiftAnimation" v-if="LiveGiftAnimation && currentGiftAnimation" :gift="currentGiftAnimation"
    ...
    <transition-group name="float" tag="div" class="absolute inset-0 pointer-events-none z-50">
      <div
        ...
        :style="{ left: like.x+'px', top: like.y+'px', animation: 'float-up 1.1s ease-out forwards' }"
      >💖</div>
    </transition-group>
    ...
    <transition name="fade">
      <div
        ...
        class="fixed bottom-[180px] left-3 z-40 bg-white/10 backdrop-blur rounded-xl p-3 text-[12px] text-white w-[92vw] max-w-md"
      >
    ...
    <transition name="fade">
      <div
        ...
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-black/80 px-3 py-3 rounded-xl text-sm font-bold text-white shadow-xl"
      >
        {{ streamAnnouncement }}
      </div>
    </transition>
    ...
    <div class="absolute bottom-40 right-3 z-50 flex flex-col items-center gap-1">
      <button
        @click.stop="tapLikeAnywhere"
        class="w-12 h-12 bg-pink-600 rounded-full grid place-items-center hover:bg-pink-700 shadow-lg active:scale-95"
      >
        💗
      </button>
    ...
    <!-- ====== FACE FILTER SELECTOR & MORE PANEL ====== -->
    <transition name="fade">
    ...
    <transition name="fade">
      <div
        ...
        class="absolute bottom-28 left-1/2 -translate-x-1/2 z-50 px-3 py-3 rounded-xl text-sm text-white bg-black/50 backdrop-blur-md shadow-lg"
      >
...
/* ---------- SAFE ASYNC HELPERS (fallback to Noop) ---------- */
...
/* ---- FIXED: use RELATIVE paths with correct casing ---- */
...
const emit = defineEmits([
  'open-ranking','featured','explore','end',
  'open-gifts','share','open-guests','open-hosts','open-more',
  'send-message','open-effects','open-summary','open-settings',
  'superchat','recharge'
])
...
function addHeart(x = window.innerWidth - 40, y = window.innerHeight - 120, big=false, delay=0){
  ...
  setTimeout(() => {
    floatingLikes.value = floatingLikes.value.filter(h => h.id !== id)
  }, 1100 + delay*1000)
}
...
async function tryPip(){
  try {
    if ('requestPictureInPicture' in videoEl.value) {
...
function toggleReplay(){ emit('open-summary'); } // wire properly in parent
function shareLive(){
...
function confirmEndStream(){
  if (confirm('End the LIVE?')) emit('end')
}
...
/* Orientation hint for landscape-only effects (optional) */
function orientationHint(){
  const isPortrait = window.matchMedia('(orientation: portrait)').matches
  if (isPortrait) {
    streamAnnouncement.value = 'Rotate for wider view ↻'
    setTimeout(()=> streamAnnouncement.value = '', 2000)
  }
}
</script>
...
/* Floating hearts */
@keyframes floatUp {
  0% { transform: translateY(0) scale(0.9); opacity: .9 }
