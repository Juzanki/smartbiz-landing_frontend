...
/** ---------------- Props (RUNTIME) ----------------
 * Tumia defineProps({ type, default }) PEKEE.
 * Usitumie withDefaults hapa.
 */
...
/** ---------------- Public API ---------------- */
function push(type: ToastType, text: string, ttlOrOpts?: number | Partial<ToastItem> & { title?: string; ttl?: number; action?: ToastAction }) {
  ...
  const item: ToastItem = {
    ...
    paused: false,
    // swipe
    dx: 0, vx: 0, _downX: null, _lastX: 0, _lastTs: t0,
...
function startTimer(t: ToastItem) {
  const tick = (ts: number) => {
    ...
    const base = (t as any)._lastTick ?? ts
    const dt = ts - base
    ;(t as any)._lastTick = ts
    t.remaining -= dt
    if (t.remaining <= 0) { dismiss(t.id); return }
    t._raf = requestAnimationFrame(tick)
  }
  ;(t as any)._lastTick = now()
  t._raf = requestAnimationFrame(tick)
}
function pause(t: ToastItem){ if (!t.paused) t.paused = true }
function resume(t: ToastItem){ if (t.paused){ t.paused = false; (t as any)._lastTick = now() } }
...
function toastStyle(t: ToastItem){
