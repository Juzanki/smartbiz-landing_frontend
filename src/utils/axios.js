...
/**
 * SmartBiz Axios (Direct Render URL)
 * - baseURL: moja kwa moja kwa Render, ikijumuisha /api
 * - Cookie auth (withCredentials) + optional Bearer
 * - Retries, ETag cache, de-dupe, offline queue
 * - Single-flight /auth/token/refresh
 *
 * Badili BASE ikiwa utabadilisha domain ya backend.
 */

// ───────────────────────────────── BaseURL (hard-coded) ─────────────────────────────────
export const BASE = "https://smartbiz-backend-p45m.onrender.com/api"; // ⟵ URL yako ya Render

const WITH_CREDENTIALS = true;          // cookies from backend
const APP_VERSION      = import.meta.env?.VITE_APP_VERSION || "0.0.0";
const DEFAULT_TIMEOUT  = 10_000;
const MAX_RETRIES      = Number(import.meta.env?.VITE_API_MAX_RETRIES || 3);
const CACHE_TTL_MS     = Number(import.meta.env?.VITE_API_CACHE_TTL_MS || 5 * 60_000);
const REFRESH_URL      = "/auth/token/refresh"; // relative to BASE
...
const tokenProvider = {
  ...
  getRefresh:  () => localStorage.getItem("refresh_token") || null,   // reserved
  setRefresh:  (t) => localStorage.setItem("refresh_token", t || ""), // reserved
...
const makeKey = (cfg) => {
  ...
  const body = (method || "get").toLowerCase() === "get"
...
const raw = axios.create({ baseURL: BASE, withCredentials: WITH_CREDENTIALS });
...
instance.interceptors.request.use((config) => {
  ...
  // Optional Bearer – cookie auth ipo via withCredentials
...
instance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error?.response?.status === 401 && !config.__isRetryAfterAuth && !config.url?.endsWith(REFRESH_URL)) {
      try {
...
const simpleApi = axios.create({ baseURL: BASE, withCredentials: WITH_CREDENTIALS });
