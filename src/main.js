...
function pickBrowserLang() {
  try { const saved = localStorage.getItem('user_lang'); if (saved) return saved } catch {}
  const nav = String((navigator?.language || 'en')).toLowerCase()
  if (nav.startsWith('sw')) return 'sw'
  if (nav.startsWith('fr')) return 'fr'
  return 'en'
}
const i18n = createI18n({ legacy:false, locale: pickBrowserLang(), fallbackLocale:'en', messages:{en,sw,fr} })
...
watch(() => i18n.global.locale.value, v => { try{localStorage.setItem('user_lang',v)}catch{}; applyLangDir(v) })

// Toasts
import Toast, { useToast } from 'vue-toastification'
import 'vue-toastification/dist/index.css'
const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator?.userAgent || '')
const toastOptions = { position: isMobile ? 'bottom-center' : 'top-right', timeout: 3500, draggable: true, maxToasts: 4 }

// Charts (optional lazy)
let __ChartJS=null; async function ensureChartJs(){ if(__ChartJS) return __ChartJS; const mod=await import('chart.js'); mod.Chart.register(...mod.registerables); return (__ChartJS=mod.Chart) }

// Global guards / diagnostics
function showBootIssue(message, err){
  const el = document.getElementById('app')
  if (el) el.innerHTML = `<div style="color:#f88;background:#111;padding:12px;border:1px solid #333;border-radius:8px;max-width:860px;margin:24px auto;font:14px system-ui">
...
app.config.errorHandler = (err,_i,info)=>{ if(import.meta.env.DEV) console.error('[AppError]',err,info); try{ window.$toast?.error?.('Unexpected error') }catch{} }

// Route helpers
function setRouteLoading(on){ try{ on ? document.body.classList.add('route-loading') : document.body.classList.remove('route-loading') }catch{} }

// Service worker (optional)
if ('serviceWorker' in navigator && import.meta.env.PROD) {
  const swUrl='/sw.js'; fetch(swUrl,{method:'HEAD'}).then(r=>{ if(r.ok) navigator.serviceWorker.register(swUrl).catch(()=>{}) }).catch(()=>{})
}

// Mount + watchdog
;(async ()=>{
  const watchdog = setTimeout(()=>{ if(!window.__APP_MOUNTED__) showBootIssue('App is taking longer than expected to start…') }, 6000)
  try{
    // 👉 Leta router kwa dynamic import; ukishindikana bado tunamount bila router
    let router = null
    try{
    ...
    }catch(e){ console.warn('Router failed to load, starting without it:', e) }
